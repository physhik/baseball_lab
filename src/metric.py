...
def fangraphs_stats(start_season, end_season=None, league='all', qual=1, ind=1):
    """
...
def statcast_stats(year):
  ...
  s_con=requests.get(url_con.format(year))
  s_brl=requests.get(url_brl.format(year))
  s_con_wo_hr = requests.get(url_con_wo_hr.format(year))
...
def new_metric(year):
  ...
  con, brl, con_wohr= statcast_stats(year)
  fg  = fangraphs_stats(year)
